...
class RoutesCompiler(file: File, generatedDir: File, additionalImports: Seq[String], generateReverseRouter: Boolean = true, generateRefReverseRouter: Boolean = true, namespaceReverseRouter: Boolean = false) {
  def execute() = RoutesCompilerObj.compile(file, generatedDir, additionalImports, generateReverseRouter, generateRefReverseRouter, namespaceReverseRouter)
}
